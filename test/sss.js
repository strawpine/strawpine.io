...
Vara.prototype.createNode = function(a, b) {
    for (var c in a = document.createElementNS("http://www.w3.org/2000/svg", a),
    b)
        a.setAttributeNS(null, c.replace(/[A-Z]/g, function(a) {
...
Vara.prototype.getSVGData = function() {
    ...
    b.onreadystatechange = function() {
        4 == b.readyState && 200 == b.status && (a.contents = JSON.parse(b.responseText),
        ...
        for (var c = [], d = "string" == typeof this.texts[b] ? {
            ...
            if (0 < a.breakPoints[b][f].length)
                for (var h, i, l = a.breakPoints[b][f].length, m = 0; m <= l; m++) {
                    ...
                    a.breakPoints[b][f][l - 1] != g.length && a.breakPoints[b][f].push(g.length);
                    var n = null == a.breakPoints[b][f][m - 1] ? 0 : a.breakPoints[b][f][m - 1];
                    h = g.slice(n, i).replace(/^\s+/g, ""),
                    c.push(h)
                }
            else
                c.push(e[f])
...
Vara.prototype.createText = function() {
    ...
    for (var g = 0, h = !0, k = 0; k < this.texts.length; k++) {
        ...
        (this.texts[k].autoAnimation == null || this.texts[k].autoAnimation) && this.properties.autoAnimation && (b.draw(q, p),
        (this.texts[k].queued == null || this.texts[k].queued) && (b.prevDuration += p))
    }
    this.completed = !0,
...
Vara.prototype.playAll = function() {
    ...
    for (var a = 0; a < this.texts.length; a++) {
        ...
        this.draw(c, b),
        (this.texts[a].queued == null || this.texts[a].queued) && (this.prevDuration += b)
    }
}
,
Vara.prototype.draw = function(a, b) {
    ...
    if (null == this.drawnCharacters[a])
        return console.warn("ID:`" + a + "` not found. Animation skipped"),
        void console.trace();
...
Vara.prototype.get = function(a) {
    this;
    return null == this.drawnCharacters[a] ? (console.warn("ID:`" + a + "` not found."),
    console.trace(),
    !1) : this.drawnCharacters[a]
}
,
...
Vara.prototype.analyseWidth = function() {
    ...
    for (var k = 0; k < this.texts.length; k++) {
        ...
        this.trueFontSize = o,
        this.texts[k].fontSize = p;
        for (var u = this.texts[k].letterSpacing, v = 0; v < l.length; v++) {
            for (var w = t, y = [], z = l[v], A = 0, B = 0; B < z.length; B++)
                if ("object" === _typeof(u) && "object" === _typeof(u) && (u = void 0 === u[z] ? void 0 === u.global ? 0 : u.global : u[z]),
                null == this.characters[z[B].charCodeAt(0)] ? " " == z[B] ? (q = this.space.w * p,
                A = w) : q = this.questionMark.w * p + u : (q = this.characters[z[B].charCodeAt(0)].w * p,
                q < this.texts[k].minWidth && (q += p * (this.texts[k].minWidth - this.characters[z[B].charCodeAt(0)].w) / 2),
                q += u),
...
Vara.prototype.setPosition = function(a, b, c) {
    ...
    g.setTranslate(e, f),
    a.transform.baseVal.replaceItem(g, 0)
}
,
